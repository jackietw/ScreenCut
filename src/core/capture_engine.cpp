...
CaptureEngine::CaptureEngine(QObject *parent) : QObject(parent) {
  ...
  connect(
      m_videoRecorder, &VideoRecorder::recordingFinished, this,
      [this](const QString &path) {
        qDebug() << "[CaptureEngine] MP4 video recording successfully saved:"
                 << path;
        emit captureSaved(path);
        Notification::showMessage(QString("🎬 Video Saved to: %1").arg(path),
                                  5000, false);
      });
  connect(m_videoRecorder, &VideoRecorder::recordingError, this,
          [](const QString &err) {
            Notification::showMessage(QString("⚠️ Recording Error: %1").arg(err),
                                      6000, true);
          });
...
void CaptureEngine::initSessionStateFromConfig() {
  if (CaptureMainWindow::instance()) {
    ...
    m_sessionSysAudioEnabled =
        CaptureMainWindow::instance()->isSettingEnabled("Record System Audio");
    m_sessionEditorEnabled =
        CaptureMainWindow::instance()->isSettingEnabled("Preview in Editor");
    m_sessionClipboardEnabled =
        CaptureMainWindow::instance()->isSettingEnabled("Copy to Clipboard");
    m_sessionImgCursorEnabled =
        CaptureMainWindow::instance()->isSettingEnabled("Image_Capture Cursor");
  } else {
    m_sessionCursorEnabled =
        Config::getValue("Video_Capture Cursor", true).toBool();
    m_sessionMicEnabled = Config::getValue("Record Microphone", true).toBool();
    m_sessionSysAudioEnabled =
        Config::getValue("Record System Audio", true).toBool();
    m_sessionEditorEnabled =
        Config::getValue("Preview in Editor", true).toBool();
    m_sessionClipboardEnabled =
        Config::getValue("Copy to Clipboard", true).toBool();
    m_sessionImgCursorEnabled =
...
void CaptureEngine::setSessionCursorEnabled(bool enabled) {
  ...
  if (m_videoRecorder && m_videoRecorder->worker()) {
    QMetaObject::invokeMethod(m_videoRecorder->worker(),
                              [w = m_videoRecorder->worker(), enabled]() {
...
void CaptureEngine::drawCursorOnRegion(QPixmap &target,
                                       const QPoint &targetTopLeftGlobal,
                                       const QPoint &cursorGlobalPos) {
  if (target.isNull())
    return;
  ...
  if (GetCursorInfo(&ci) && ci.hCursor) {
    ...
    if (GetIconInfo(ci.hCursor, &ii)) {
      ...
      if (hDib) {
        ...
        QImage cursorImg(static_cast<const uchar *>(pBits), w, h,
        ...
        if (CaptureEngine::instance()->isSessionAnimationEnabled()) {
          painter.setRenderHint(QPainter::Antialiasing, true);
          int ringRadius =
              26 + ((QDateTime::currentMSecsSinceEpoch() / 80) % 8);
  ...
  painter.setPen(
      QPen(Qt::black, 1.5, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
...
void CaptureEngine::startCapture(CaptureMode mode) {
  qDebug() << "[CaptureEngine] Starting capture. Mode:"
  ...
  if (wasVisible) {
    qDebug() << "[CaptureEngine] Hiding visible CaptureMainWindow before "
                "capture starts...";
...
void CaptureEngine::doActualCapture(CaptureMode mode) {
  ...
  qDebug() << "[CaptureEngine] Desktop snapshot captured. Size:"
           << m_fullDesktopSnapshot.size();
  ...
  m_overlayWidget =
      new RegionSelectWidget(m_fullDesktopSnapshot, m_currentMode);

  auto startScrollCaptureFunc = [this](const QRect &rect) {
    qDebug() << "[CaptureEngine] Region selected for Scrolling capture. "
                "Launching ScrollCaptureManager on rect:"
             << rect;
    if (m_overlayWidget) {
    ...
    connect(
        scrollMgr, &ScrollCaptureManager::completed, this,
        [this](const QPixmap &stitchedPix) {
          qDebug()
              << "[CaptureEngine] Scrolling capture completed. Stitched size:"
              << stitchedPix.size();
  ...
  connect(m_overlayWidget, &RegionSelectWidget::confirmRequested, this,
          [this, startScrollCaptureFunc](const QRect &rect) {
            if (m_currentMode == CaptureMode::Scrolling) {
              startScrollCaptureFunc(rect);
              return;
            }
            if (m_currentMode == CaptureMode::VideoRegion) {
              qDebug() << "[CaptureEngine] Video recording finished/confirmed "
                          "for region:"
                       << rect;
              if (CaptureEngine::instance()->videoRecorder() &&
                  CaptureEngine::instance()->videoRecorder()->isRecording()) {
  ...
  connect(m_overlayWidget, &RegionSelectWidget::regionSelected, this,
          [this, startScrollCaptureFunc](const QRect &rect) {
            if (m_currentMode == CaptureMode::Scrolling) {
              startScrollCaptureFunc(rect);
              return;
  ...
  connect(m_overlayWidget, &RegionSelectWidget::copyRequested, this,
          [this, startScrollCaptureFunc](const QRect &rect) {
            if (m_currentMode == CaptureMode::Scrolling) {
              startScrollCaptureFunc(rect);
              return;
  ...
  connect(m_overlayWidget, &RegionSelectWidget::saveRequested, this,
          [this, startScrollCaptureFunc](const QRect &rect) {
            if (m_currentMode == CaptureMode::Scrolling) {
              startScrollCaptureFunc(rect);
              return;
            ...
            QString defaultPath =
                ScutProject::getLibraryDir() + "/" +
                QString("Capture_%1.scut")
                    .arg(QDateTime::currentDateTime().toString(
                        "yyyyMMdd_hhmmss_zzz"));
            QString fileName = QFileDialog::getSaveFileName(
                nullptr, "Save ScreenCut Capture / Project", defaultPath,
                "ScreenCut Project (*.scut);;PNG Image (*.png);;JPEG Image "
                "(*.jpg);;WebP Image (*.webp)");
            if (!fileName.isEmpty()) {
              bool saved = false;
              ...
              if (saved) {
                ...
                QMessageBox::critical(
                    nullptr, "Save Error",
                    "Failed to save capture to specified location.");
              }
            } else {
              emit captureCancelled();
            }
...
#if defined(Q_OS_MACOS) || defined(Q_OS_MAC)
  // On macOS, calling showFullScreen() creates a separate Virtual Desktop Space
  // and triggers desktop swipe animation / top notch white borders.
  // Using setGeometry() + show() with Frameless/Tool flags pops the overlay
  // instantly over the current desktop without switching spaces.
...
void CaptureEngine::startWindowCapture() { startCapture(CaptureMode::Window); }

void CaptureEngine::startScrollingCapture() {
  // For Scrolling capture, we initiate with Region capture to let user select
...
QPixmap CaptureEngine::captureAllScreens() {
  QList<QScreen *> screens = QGuiApplication::screens();
  qDebug() << "[CaptureEngine] captureAllScreens across" << screens.size()
           << "monitor(s).";
  if (screens.isEmpty()) {
  ...
  qDebug() << "[CaptureEngine] Multi-monitor virtual geometry:"
           << virtualGeometry;
...
WindowInfo CaptureEngine::findWindowAt(const QPoint &globalPos) {
  ...
  if (GetCursorPos(&cursorPos)) {
    if (qAbs(globalPos.x() - qRound(cursorPos.x / dpr)) <= 5 &&
        qAbs(globalPos.y() - qRound(cursorPos.y / dpr)) <= 5) {
      physPt = cursorPos;
    }
  }

  FindWindowContext ctx = {
      physPt, GetCurrentProcessId(), nullptr, {0, 0, 0, 0}};
  ...
  info.isValid =
      !info.rect.isEmpty() && info.rect.width() > 10 && info.rect.height() > 10;
#else
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
  ...
  if (windowList) {
    ...
    for (CFIndex i = 0; i < count; ++i) {
      ...
      if (cgX >= bounds.origin.x && cgX < bounds.origin.x + bounds.size.width &&
        ...
        // Window title
        CFStringRef nameRef =
            static_cast<CFStringRef>(CFDictionaryGetValue(dict, kCGWindowName));
        if (nameRef) {
          char buf[512] = {};
  ...
  info.rect = QRect(0, 0, m_fullDesktopSnapshot.width(),
                    m_fullDesktopSnapshot.height());
  info.isValid = false;
#endif
#endif
  return info;
...
public:
  explicit VideoBorderOverlay(const QRect &logicalRect,
    ...
    // Position slightly outside the recording rect so the 2px red border wraps
    // tightly around m_selectedRect
    setGeometry(logicalRect.left() - 2, logicalRect.top() - 2,
                logicalRect.width() + 4, logicalRect.height() + 4);
  }
...
protected:
  void paintEvent(QPaintEvent *event) override {
    ...
    QPen borderPen(QColor(235, 50, 50), 2, Qt::SolidLine, Qt::RoundCap,
                   Qt::RoundJoin);
...
RegionSelectWidget::RegionSelectWidget(const QPixmap &desktopSnapshot,
                                       CaptureMode mode, QWidget *parent)
    : QWidget(parent), m_desktopSnapshot(desktopSnapshot), m_mode(mode) {
  setWindowFlags(Qt::FramelessWindowHint | Qt::WindowStaysOnTopHint | Qt::Tool |
                 Qt::NoDropShadowWindowHint | Qt::BypassWindowManagerHint);
...
void RegionSelectWidget::setVideoRecordingActive(bool active) {
  ...
  if (active) {
    ...
    hide(); // Hide full-screen selection widget so clicks pass directly to
            // desktop and separate border overlay shows the red border
  } else {
    if (m_borderOverlay) {
      m_borderOverlay->close();
      m_borderOverlay->deleteLater();
...
void RegionSelectWidget::paintEvent(QPaintEvent * /*event*/) {
  ...
  if (m_selectionConfirmed && m_selectedRect.isValid()) {
    ...
    if ((m_currentMousePos - m_startPoint).manhattanLength() > 10 &&
        (dragRect.width() > 10 || dragRect.height() > 10)) {
  ...
  if (highlightRect.isValid() && !highlightRect.isEmpty()) {
    ...
    if (m_isCountingDown && highlightRect == m_selectedRect) {
      ...
      painter.drawText(highlightRect, Qt::AlignCenter,
      ...
      painter.drawRect(highlightRect.left() - handleSize / 2,
                       highlightRect.top() - handleSize / 2, handleSize,
                       handleSize);
      painter.drawRect(highlightRect.right() - handleSize / 2,
                       highlightRect.top() - handleSize / 2, handleSize,
                       handleSize);
      painter.drawRect(highlightRect.left() - handleSize / 2,
                       highlightRect.bottom() - handleSize / 2, handleSize,
                       handleSize);
      painter.drawRect(highlightRect.right() - handleSize / 2,
                       highlightRect.bottom() - handleSize / 2, handleSize,
                       handleSize);
...
void RegionSelectWidget::drawDimensionsTooltip(QPainter &painter,
                                               const QRect &rect) {
  ...
  QRect tooltipRect(rect.left(), rect.top() - textHeight - 12, textWidth + 16,
                    textHeight + 8);
...
void RegionSelectWidget::drawMagnifier(QPainter &painter, const QPoint &pos) {
  ...
  painter.save();
  QColor themeColor = (m_mode == CaptureMode::VideoRegion)
                          ? QColor(235, 50, 50, 180)
                          : QColor(0, 168, 255, 180);
  ...
  clipPath.addRoundedRect(loupePos.x(), loupePos.y(), loupeSize, loupeSize,
  ...
  QRectF sourceRect((pos.x() - logicalSourceSize / 2.0) * dpr,
                    ...
                    logicalSourceSize * dpr, logicalSourceSize * dpr);
  ...
  painter.drawLine(loupePos.x() + loupeSize / 2, loupePos.y(),
                   loupePos.x() + loupeSize / 2, qRound(centerBox.top()));
  painter.drawLine(loupePos.x() + loupeSize / 2, qRound(centerBox.bottom()),
                   loupePos.x() + loupeSize / 2, loupePos.y() + loupeSize);
  painter.drawLine(loupePos.x(), loupePos.y() + loupeSize / 2,
                   qRound(centerBox.left()), loupePos.y() + loupeSize / 2);
  painter.drawLine(qRound(centerBox.right()), loupePos.y() + loupeSize / 2,
                   loupePos.x() + loupeSize, loupePos.y() + loupeSize / 2);
  painter.restore();

  // Draw border around loupe
  painter.setPen(QPen(QColor(255, 255, 255, 220), 2));
  painter.setBrush(Qt::NoBrush);
  painter.drawRoundedRect(loupePos.x(), loupePos.y(), loupeSize, loupeSize,
                          loupeSize / 2.0, loupeSize / 2.0);
...
void RegionSelectWidget::mousePressEvent(QMouseEvent *event) {
  if (m_isCountingDown || m_isVideoRecordingActive ||
      (CaptureEngine::instance()->videoRecorder() &&
       CaptureEngine::instance()->videoRecorder()->isRecording())) {
    return;
  }
...
void RegionSelectWidget::mouseMoveEvent(QMouseEvent *event) {
  ...
  m_currentMousePos = event->pos();
  if (m_isSelecting ||
      (m_mode == CaptureMode::Window || m_mode == CaptureMode::Region ||
       m_mode == CaptureMode::VideoRegion)) {
    m_lastSelectionMousePos = event->pos();
  }
...
void RegionSelectWidget::mouseReleaseEvent(QMouseEvent *event) {
  if (event->button() == Qt::LeftButton) {
    ...
    if (m_isSelecting) {
      ...
      if ((m_endPoint - m_startPoint).manhattanLength() > 10 &&
          (draggedRect.width() > 10 || draggedRect.height() > 10)) {
        // 點一下並拖曳 為自訂大小
...
void RegionSelectWidget::mouseDoubleClickEvent(QMouseEvent *event) {
  if (event->button() == Qt::LeftButton && m_selectionConfirmed &&
      m_selectedRect.contains(event->pos())) {
    m_lastSelectionMousePos = event->pos();
    executeConfirm();
  }
}
...
int RegionSelectWidget::hitTestHandle(const QPoint &pos) const {
  ...
  if (QRect(r.topLeft() - QPoint(s / 2, s / 2), QSize(s, s)).contains(pos))
    return 1;
  if (QRect(r.topRight() - QPoint(s / 2, s / 2), QSize(s, s)).contains(pos))
    return 2;
  if (QRect(r.bottomLeft() - QPoint(s / 2, s / 2), QSize(s, s)).contains(pos))
    return 3;
  if (QRect(r.bottomRight() - QPoint(s / 2, s / 2), QSize(s, s)).contains(pos))
    return 4;
  if (QRect(r.left() + s, r.top() - s / 2, r.width() - 2 * s, s).contains(pos))
    return 5;
  if (QRect(r.left() + s, r.bottom() - s / 2, r.width() - 2 * s, s)
          .contains(pos))
    return 6;
  if (QRect(r.left() - s / 2, r.top() + s, s, r.height() - 2 * s).contains(pos))
    return 7;
  if (QRect(r.right() - s / 2, r.top() + s, s, r.height() - 2 * s)
          .contains(pos))
    return 8;
  if (r.contains(pos))
    return 9;
...
void RegionSelectWidget::drawResizeHandles(QPainter &painter,
                                           const QRect &rect) {
  if (!m_selectionConfirmed)
    return;
  painter.save();
  QColor handleColor = (m_mode == CaptureMode::VideoRegion)
                           ? QColor(235, 50, 50)
                           : QColor("#00a8ff");
...
void RegionSelectWidget::updateToolbarPosition() {
  if (!m_toolbar || !m_selectionConfirmed)
    return;
...
void RegionSelectWidget::ensureToolbarAndShow() {
  if (!m_toolbar) {
    ...
    connect(m_toolbar, &CaptureToolBarWidget::actionConfirm, this,
            &RegionSelectWidget::executeConfirm);
    connect(m_toolbar, &CaptureToolBarWidget::actionEdit, this,
            &RegionSelectWidget::executeEdit);
    connect(m_toolbar, &CaptureToolBarWidget::actionCopy, this,
            &RegionSelectWidget::executeCopy);
    connect(m_toolbar, &CaptureToolBarWidget::actionSave, this,
            &RegionSelectWidget::executeSave);
    connect(m_toolbar, &CaptureToolBarWidget::actionPin, this,
            &RegionSelectWidget::executePin);
    connect(m_toolbar, &CaptureToolBarWidget::actionCancel, this,
            &RegionSelectWidget::cancelled);
    connect(m_toolbar, &CaptureToolBarWidget::actionVideoRecordStateChanged,
            this, [this](bool rec) {
              if (rec) {
                startVideoCountdown();
              } else {
    ...
    connect(m_toolbar, &CaptureToolBarWidget::actionVideoStop, this,
            [this]() { executeConfirm(); });
    connect(m_toolbar, &CaptureToolBarWidget::cursorToggled, this,
            [this](bool checked) {
              CaptureEngine::instance()->setSessionCursorEnabled(checked);
              if (m_statusCard)
                m_statusCard->refreshStatus();
            });
    connect(m_toolbar, &CaptureToolBarWidget::micToggled, this,
            [this](bool checked) {
              CaptureEngine::instance()->setSessionMicEnabled(checked);
              if (m_statusCard)
                m_statusCard->refreshStatus();
            });
    connect(m_toolbar, &CaptureToolBarWidget::sysAudioToggled, this,
            [this](bool checked) {
...
void RegionSelectWidget::executePin() {
  if (m_selectedRect.isValid() && !m_selectedRect.isEmpty()) {
    ...
    if (CaptureEngine::instance()->isSessionImgCursorEnabled()) {
      QPoint cursorPos = m_lastSelectionMousePos.isNull()
                             ? QCursor::pos()
                             : m_lastSelectionMousePos;
      CaptureEngine::instance()->drawCursorOnRegion(
          cropped, m_selectedRect.topLeft(), cursorPos);
    }
...
// ============================================================================
// VideoCaptureWorker & VideoRecorder Implementation (Integrated into
// CaptureEngine)
// ============================================================================
...
static QList<std::function<void(const QList<HwEncoderInfo> &)>>
    s_detectCallbacks;

QString VideoRecorder::getFFmpegPath() {
#ifdef Q_OS_WIN
  const QString exeName = "ffmpeg.exe";
#else
  const QString exeName = "ffmpeg";
#endif

  QString appDirExe = QApplication::applicationDirPath() + "/" + exeName;
  if (QFile::exists(appDirExe))
    return appDirExe;

  QString resourceExe =
      QApplication::applicationDirPath() + "/../Resources/" + exeName;
  if (QFile::exists(resourceExe))
    return QFileInfo(resourceExe).absoluteFilePath();
  ...
  QString pathExe = QStandardPaths::findExecutable(exeName, searchPaths);
  if (!pathExe.isEmpty())
    return pathExe;
  ...
  if (appDataDir.exists()) {
    QStringList entries =
        appDataDir.entryList(QStringList() << "*ffmpeg*", QDir::Files);
    if (!entries.isEmpty()) {
...
QList<HwEncoderInfo>
VideoRecorder::detectAvailableHwEncoders(bool forceRefresh) {
  s_hwMutex.lock();
  ...
  qDebug()
      << "[VideoRecorder] Detecting supported hardware encoders using FFmpeg:"
      << exe;
  ...
  for (const auto &cand : candidates) {
    ...
    process.setProcessChannelMode(
        QProcess::MergedChannels); // Capture stdout/stderr internally to keep
                                   // console clean
    process.start(exe, args);
    bool finished = process.waitForFinished(3000);
    int ret = (finished && process.exitStatus() == QProcess::NormalExit)
                  ? process.exitCode()
                  : -1;

    if (ret == 0) {
      qDebug() << "[VideoRecorder] Hardware encoder supported:" << cand.codec
               << "(" << cand.name << ")";
      tempEncoders.append(HwEncoderInfo{cand.codec, cand.name});
    } else {
      qDebug() << "[VideoRecorder] Hardware encoder not supported on this PC "
                  "(filtered out):"
               << cand.codec;
    }
  }
...
void VideoRecorder::detectAvailableHwEncodersAsync(
    std::function<void(const QList<HwEncoderInfo> &)> callback,
    bool forceRefresh) {
  s_hwMutex.lock();
  if (s_hwDetected && !forceRefresh) {
    QList<HwEncoderInfo> copy = s_cachedHwEncoders;
  ...
  qDebug() << "[VideoRecorder] Starting asynchronous GPU hardware encoder "
              "probing in background thread...";
...
VideoCaptureWorker::VideoCaptureWorker(const QRect &rect,
                                       const QString &outputPath,
                                       bool captureCursor, QObject *parent)
    : QObject(parent), m_rect(rect), m_outputPath(outputPath),
...
void VideoCaptureWorker::startRecording() {
  ...
  if (limit1080p && (targetW > 1920 || targetH > 1080)) {
    QSize scaled =
        QSize(targetW, targetH).scaled(1920, 1080, Qt::KeepAspectRatio);
    targetW = scaled.width();
    targetH = scaled.height();
    qDebug() << "[VideoRecorder] Record Limit 1080p active. Downscaling "
                "resolution from"
             << m_rect.size() << "to" << targetW << "x" << targetH;
  }
  ...
  qDebug() << "[VideoRecorder] Starting MP4 recording using ffmpeg:" << ffmpeg
           << "Codec:" << codec << "(HW acceleration:" << isHw
  ...
  QString
      micDeviceName; // FFmpeg device name string (empty = default or disabled)
  if (micEnabled) {
    QByteArray savedMicId =
        Config::getValue("mic_device", QString()).toString().toUtf8();
  ...
  if (micEnabled && !micDeviceName.isEmpty()) {
    ...
    qDebug() << "[VideoRecorder] Microphone audio input enabled. Device:"
             << micDeviceName;
  } else if (micEnabled) {
    qDebug() << "[VideoRecorder] Microphone enabled but no device found. "
                "Recording video only.";
  } else {
    qDebug() << "[VideoRecorder] Microphone disabled. Recording video only (no "
                "audio).";
  }
  ...
  args << "-y"
       ...
       << "-vcodec" << "rawvideo"
       << "-s"
       << QString("%1x%2").arg(m_targetSize.width()).arg(m_targetSize.height())
       << "-pix_fmt" << "bgra"
       << "-r" << "30"
       << "-i" << "-"
  ...
  if (!m_ffmpegProcess->waitForStarted(3000)) {
    qWarning() << "[VideoRecorder] Failed to start ffmpeg process:"
               << m_ffmpegProcess->errorString();
    // If specific hw codec failed, attempt software compression fallback
    if (isHw) {
      qDebug() << "[VideoRecorder] Hardware acceleration failed to start, "
      ...
      args << "-y" << "-f" << "rawvideo" << "-vcodec" << "rawvideo"
           ...
           << "-pix_fmt" << "yuv420p" << m_outputPath;
      m_ffmpegProcess->start(ffmpeg, args);
      if (!m_ffmpegProcess->waitForStarted(3000)) {
        emit errorOccurred("Cannot start FFmpeg process for MP4 encoding. "
                           "Please check ffmpeg installation.");
        return;
      }
    } else {
      emit errorOccurred("Cannot start FFmpeg process for MP4 encoding. Please "
...
void VideoCaptureWorker::captureFrame() {
  if (!m_isRunning || !m_ffmpegProcess ||
      m_ffmpegProcess->state() != QProcess::Running) {
    return;
  }
  ...
  QPixmap snapshot = screen->grabWindow(0, m_rect.x(), m_rect.y(),
                                        m_rect.width(), m_rect.height());
  if (m_captureCursor) {
    CaptureEngine::instance()->drawCursorOnRegion(snapshot, m_rect.topLeft(),
                                                  QCursor::pos());
  }

  QImage img = snapshot.toImage().convertToFormat(QImage::Format_ARGB32);
  if (img.size() != m_targetSize) {
    img = img.scaled(m_targetSize, Qt::IgnoreAspectRatio,
                     Qt::SmoothTransformation);
  }
  m_ffmpegProcess->write(reinterpret_cast<const char *>(img.constBits()),
                         img.sizeInBytes());
}
...
void VideoCaptureWorker::stopRecording() {
  ...
  qDebug() << "[VideoRecorder] Recording completed and saved to:"
           << m_outputPath;
...
void VideoRecorder::start(const QRect &rect) {
  ...
  QString outputPath =
      QDir(libraryDir).filePath("ScreenCut_Video_" + timeStr + ".mp4");
  ...
  connect(m_workerThread, &QThread::started, m_worker,
          &VideoCaptureWorker::startRecording);
  connect(m_worker, &VideoCaptureWorker::finished, this,
          [this](const QString &path) {
            m_isRecording = false;
            if (m_workerThread) {
              m_workerThread->quit();
  ...
  connect(m_worker, &VideoCaptureWorker::errorOccurred, this,
